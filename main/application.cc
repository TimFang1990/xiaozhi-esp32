...
#if CONFIG_USE_WAKE_WORD_DETECT
    #ifndef CONFIG_USE_WAKENET_DIRECT_IF
    wake_word_detect_ = std::make_unique<WakeWordDetect>();
...
#if CONFIG_USE_WAKE_WORD_DETECT
    #ifdef CONFIG_USE_WAKENET_DIRECT_IF
    if(wake_up_word_detected_){
        if(wake_word_detect_ != nullptr){
            wake_word_detect_ = nullptr; //It's said that this will be auto released when nullptr is assigned
            ESP_LOGI(TAG, "Wake up detector is released after wake up word is detected.");
        }
        Schedule([this](){
            ResetDecoder();
            ...
            SetDeviceState(kDeviceStateConnecting);
            if (!protocol_ || !protocol_->OpenAudioChannel()) {
                ESP_LOGI(TAG, "Failed to OpenAudioChannel.");
                return;
            }
            SetListeningMode(realtime_chat_enabled_ ? kListeningModeRealtime : kListeningModeAutoStop);
        });
        
        wake_up_word_detected_ = false;
    }
    #endif

    if (wake_word_detect_ != nullptr && wake_word_detect_->IsDetectionRunning()) {
...
void Application::SetDeviceState(DeviceState state) {
    ...
    switch (state) {
        ...
        case kDeviceStateIdle:
            ...
            if(wake_word_detect_ == nullptr){
                wake_word_detect_ = std::make_unique<WakeWordDetect>();
                wake_word_detect_->Initialize(Board::GetInstance().GetAudioCodec());
                wake_word_detect_->OnWakeWordDetected([this](const std::string& wake_word) {
                    Schedule([this, &wake_word]() {
                        ESP_LOGI(TAG, "Wake word detected: %s", wake_word.c_str());
                        wake_up_word_detected_ = true;
                    });
                });
                wake_word_detect_->StartDetection();
                ESP_LOGI(TAG, "Wake up detector is created after wake up word is detected.");
            }
    #else
            wake_word_detect_->StartDetection();
    #endif
